---
title: 前端理论基础
date: 2022-11-24 16:11:51
permalink: /pages/643172/
categories:
  - 更多
  - 面试
tags:
...
key是给每一个vnode的唯一id,也可以依靠key更准确，更快的拿到oldVnode中对应的vnode节点
...
:::note
 webpack 相关知识可以看鲨鱼哥这篇文章 [前端进阶高薪必看-Webpack 篇](/pages/c8f9cb/) 说的很通俗易懂 基本应对简单的面试足够了
:::
...
* bind 是返回绑定 **this 之后的函数**，便于稍后调用；apply 、call 则是**立即执行** 。
...
### 11、http 状态码 204 301 302 304 400 401 403 404 含义
...
* fixed可以简单说 fixed 是特殊版的 absolute，fixed 元素总是相对于 body 定位的。 **使用场景：** 侧边栏或者广告图


* inherit继承父元素的 position 属性，但需要注意的是 IE8 以及往前的版本都不支持 inherit 属性。


* sticky设置了 sticky 的元素，在屏幕范围（viewport）时该元素的位置并不受到定位影响（设置是 top、left 等属性无效），当该元素的位置将要移出偏移范围时，定位又会变成 fixed，根据设置的 left、top 等属性成固定位置的效果。当元素在容器中被滚动超过指定的偏移值时，元素在容器内固定在指定位置。亦即如果你设置了 top: 50px，那么在 sticky 元素到达距离相对定位的元素顶部 50px 的位置时固定，不再向上移动（相当于此时 fixed 定位）。
**使用场景：** 跟随窗口
...
### 17、Vue diff 原理
![image](https://cdn.staticaly.com/gh/HeQingHua2018/image-hosting@master/20221124/image.1hs43xtqll34.webp)
...
### 3 require 具体实现原理是什么
**require 基本原理** 
![image](https://cdn.staticaly.com/gh/HeQingHua2018/image-hosting@master/20221124/image.6hcrnrzhp200.webp)

**require 查找路径** 
![image](https://cdn.staticaly.com/gh/HeQingHua2018/image-hosting@master/20221124/image.45s34e5zfqk.webp)
